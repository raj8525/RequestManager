...
标题和正文均为必填文本字段：标题去除首尾空白后为 1-100 个字符，正文为 10-10,000 个字符。列表显示独立标题；历史无标题数据的兼容和一次性补录规则见 `docs/superpowers/specs/2026-07-14-request-titles-design.md`。
...
开发者默认按最后更新时间倒序。双方都可搜索编号、标题和正文，并按项目、类型、优先级、进度、记录状态筛选；客户只能选择所属项目。每页 25 条。归档默认隐藏，但筛选入口始终可发现。
